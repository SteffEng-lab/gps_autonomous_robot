...
class Sensor_Fusion(Node):
    def __init__(self):
        ...
        #cov_R = np.array([[3**2, 0], [0, 2**2]])       # Values from noise analysis
        cov_R = np.array([[1.255**2, 0], [0, 1.599**2]])
        cov_Q_process = np.diag([0**2, 0**2, 0**2, 0**2])         
        ...
        # Heading in ENU world frame (0 = East, π/2 = North).
        # Set to the robot's actual initial heading before starting.
        #self.heading = 0.0          # east
        self.heading = np.pi / 2    # north

        self.gps_average_counter = 0
        self.GPS_AVERAGE_MAX_STEPS = 10
        self.gps_average_readings = np.zeros((self.GPS_AVERAGE_MAX_STEPS, 2))

        self.is_gps_initialized = False
        ...
        if msg.status.status >= 0:      # GPS fix available
            ...
            if self.lat_0 is None:
                ...
                if self.gps_average_counter < self.GPS_AVERAGE_MAX_STEPS:
                    ...
                    # Position uncertainty = GPS accuracy, velocity uncertainty = 0
                    # (robot starts from rest). Keeps K[v,x] ≈ 0 so GPS noise cannot
                    # inject spurious velocity — velocity comes from IMU integration only.
                    self.kalman_filter.est_error_cov = np.diag([2.0**2, 2.0**2, 0.0, 0.0])
                    self.is_gps_initialized = True
    ...
    def imu_callback(self, msg):
        ...
        if not self.is_imu_calibrated:
            if self.imu_calibration_counter >= self.IMU_CALIBRATION_MAX_STEPS:
                ...
                self.gyro_z_bias = self.gyro_z_sum  / self.IMU_CALIBRATION_MAX_STEPS
                self.is_imu_calibrated = True
                self.get_logger().info(
                    f"IMU calibrated. Bias: ax={self.imu_bias[0]:.5f}, "
                    f"ay={self.imu_bias[1]:.5f}, gyro_z={self.gyro_z_bias:.5f}"
                )
            else:
        ...
        if not self.is_gps_initialized:
            return

        # IMU z points DOWN → negate gyro_z to get ENU (z-up) yaw rate
        self.heading += -(gyro_z - self.gyro_z_bias) * self.dt

        # Rotate IMU acceleration from body frame into ENU world frame.
        # IMU: y=forward, x=right, z=down. Heading = angle of forward (body-y) from East, CCW.
        # R = [[sin, cos], [-cos, sin]]
        cos_h = np.cos(self.heading)
        sin_h = np.sin(self.heading)
        acc_x_corrected = acc_x - self.imu_bias[0]
        acc_y_corrected = acc_y - self.imu_bias[1]
        acc_x_world =  sin_h * acc_x_corrected + cos_h * acc_y_corrected
        acc_y_world = -cos_h * acc_x_corrected + sin_h * acc_y_corrected

        self.kalman_filter.current_u = np.array([[acc_x_world], [acc_y_world]])
        ...
        odom_msg.pose.pose.position.x = float(self.kalman_filter.pred_state[0, 0])
        odom_msg.pose.pose.position.y = float(self.kalman_filter.pred_state[1, 0])

        # Publish heading as quaternion (2D rotation around z axis)
        odom_msg.pose.pose.orientation.z = float(np.sin(self.heading / 2))
        odom_msg.pose.pose.orientation.w = float(np.cos(self.heading / 2))

        odom_msg.twist.twist.linear.x = float(self.kalman_filter.pred_state[2, 0])
        odom_msg.twist.twist.linear.y = float(self.kalman_filter.pred_state[3, 0])
